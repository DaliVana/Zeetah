...
pub const Backtracker = struct {
    ...
    /// Min/max byte width a node can match. `bounded == false` means the max is
    /// unbounded (`*`/`+`/backref) — callers cap it (e.g. at `pos` for a
    /// lookbehind). Used to drive the variable-width lookbehind reverse scan.
    const WidthBounds = struct { min: usize, max: usize, bounded: bool };
    fn widthBounds(self: *Backtracker, ref: NodeRef) WidthBounds {
    ...
    fn tick(self: *Backtracker) Error!void {
        self.steps += 1;
    ...
    inline fn enter(self: *Backtracker) Error!void {
    ...
    fn m(self: *Backtracker, ref: NodeRef, pos: usize, k: *const Cont) Error!bool {
        try self.enter();
        defer self.depth -= 1;
        try self.tick();
        const nd = self.h.node(ref);
        switch (nd.tag) {
            ...
            .look_around => {
                ...
                if (!behind) {
                    ...
                    // candidate start offsets `w` (shortest first, so a negative
                    // lookbehind rejects on the first violating span). For a
                    // fixed-width sub-pattern this collapses to a single offset —
                    // byte-identical to the previous fixed-only path. Unbounded
                    // `*`/`+`/backref cap at `pos` (nothing precedes index 0).
                    // Every `m` step ticks the same anti-ReDoS budget, so this
                    // stays O(pos·sub) and bounded — never a hang.
    ...
    fn loopStep(self: *Backtracker, pos: usize, lp: *const Cont) Error!bool {
        try self.enter();
        defer self.depth -= 1;
    ...
    fn matchAtomic(self: *Backtracker, body: NodeRef, pos: usize, k: *const Cont) Error!bool {
    ...
    fn cont(self: *Backtracker, pos: usize, k: *const Cont) Error!bool {
        try self.enter();
        defer self.depth -= 1;
        try self.tick();
        switch (k.*) {
